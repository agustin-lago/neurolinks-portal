      ...
      if (domainInfo) {
        ...
        for (const record of dnsRecords) {
          if (record.recordType === "DNS_RECORD_TYPE_CNAME" && record.requiredValue && record.fqdn) {
            ...
            if (!railwayPublicUrl) {
              railwayPublicUrl = record.requiredValue;
            }
          }
        }
        ...
        // Expose public URL in deploy return value
        console.log(`[deployBackoffice] Captured Railway public URL: "${railwayPublicUrl}"`);
        return {
          domain: `${slug}.clientesneurolinks.com`,
          projectId: projectRwId,
...
export async function activateClientPortal(clienteId, adminDb) {
  ...
  const { data: lockAcquired, error: lockErr } = await adminDb
    ...
    .select("id, plan_tipo, lineas_cantidad, deployment_urls, tokens_backoffice, backoffice_activado, proyecto_slug, plan, abono, vendedor_id");
