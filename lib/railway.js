...
export async function deployBackoffice({ slug, planTipo, existingProjectId, onProjectCreated }) {
...
export async function activateClientPortal(clienteId, adminDb) {
  ...
  const { data: lockAcquired, error: lockErr } = await adminDb
    ...
    .select("id, plan_tipo, lineas_cantidad, deployment_urls, tokens_backoffice, backoffice_activado, proyecto_slug, plan, abono, vendedor_id, auth_user_id");
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (existingProjectId && cliente.backoffice_activado) {
        ...
        // Ensure it is linked in proyectos_railway (in case it got unlinked/missing)
        try {
          await adminDb
            .from('proyectos_railway')
            .upsert(
              { railway_project_id: existingProjectId, cliente_id: clienteId },
              { onConflict: 'railway_project_id' }
            );
        } catch (linkErr) {
          console.error(`[activateClientPortal] Failed to link existing project in proyectos_railway:`, linkErr);
        }
        continue;
      }
          ...
          onProjectCreated: async (newProjectId) => {
            ...
            try {
              ...
              await adminDb
                .from('proyectos_railway')
                .upsert(
                  { railway_project_id: newProjectId, cliente_id: clienteId },
                  { onConflict: 'railway_project_id' }
                );
              console.log(`[activateClientPortal] Vinculación instantánea completada en Supabase para el cliente ${clienteId}`);
            } catch (instErr) {
      ...
      // Link project to client in proyectos_railway (enables multi-instance support)
      if (projectId) {
        try {
          await adminDb
            .from('proyectos_railway')
            .upsert(
              { railway_project_id: projectId, cliente_id: clienteId },
              { onConflict: 'railway_project_id' }
            );
          console.log(`[activateClientPortal] Linked project ${projectId} to client ${clienteId} in proyectos_railway`);
        } catch (linkErr) {
          console.error(`[activateClientPortal] Failed to link project in proyectos_railway:`, linkErr);
        }
      }
