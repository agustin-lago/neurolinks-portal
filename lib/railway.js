...
export async function updateRailwayProjectName(projectId, newName) {
  const cleanName = String(newName || "").trim();
  if (!projectId) throw new Error("Falta el ID del proyecto Railway.");
  if (!cleanName) throw new Error("El nombre del proyecto no puede estar vacio.");
  ...
  `, { id: projectId, input: { name: cleanName } });

  return result?.projectUpdate || null;
}
export async function deployBackoffice({ slug, planTipo, proyectoNombre, existingProjectId, onProjectCreated }) {
  ...
  const defaultName = cleanSlug ? (["standar", "standard", "masivo_meta"].includes(String(planTipo || "").toLowerCase()) ? `CRM - ${cleanSlug}` : `BOT - ${cleanSlug}`) : (proyectoNombre || `Backoffice - ${existingProjectId || "manual"}`);
...
export async function activateClientPortal(projectRowId, adminDb) {
  const { data: lockAcquired, error: lockErr } = await adminDb
    ...
    .select("id, cliente_id, railway_project_id, deployment_url, railway_public_url, backoffice_activado, proyecto_slug, nombre_personalizado, clientes ( auth_user_id, vendedor_id, plan_tipo, lineas_cantidad, plan )");
  ...
  try {
    try {
      ...
      const deployResult = await deployBackoffice({
        ...
        planTipo: projectRow.clientes?.plan_tipo,
    ...
    if (projectRow.clientes?.vendedor_id) {
      await recalculatePlanSubscriptions(projectRow.clientes.vendedor_id, projectRow.clientes?.plan_tipo, projectRow.clientes?.lineas_cantidad, adminDb);
    }
