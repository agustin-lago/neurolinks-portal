...
import { createCnameRecord } from "./dns.js";
...
export async function deployBackoffice({ slug, planTipo, existingProjectId }) {
  ...
  // 6.1 — Query CNAME validation requiredValue and automate DNS registration
  if (domainCreated) {
    try {
      console.log(`[deployBackoffice] Step 6.1: Querying CNAME requiredValue for domain validation...`);
      ...
      if (domainInfo) {
        const cnameRecord = domainInfo.status?.dnsRecords?.find(r => r.recordType === "DNS_RECORD_TYPE_CNAME");
        if (cnameRecord && cnameRecord.requiredValue) {
          console.log(`[deployBackoffice] Found CNAME target required value: "${cnameRecord.requiredValue}"`);
          // Call Hostinger DNS API to register CNAME
          createCnameRecord({ slug, requiredValue: cnameRecord.requiredValue })
            .then(success => {
              if (success) {
                console.log(`[deployBackoffice] Automated DNS creation completed successfully.`);
              } else {
          ...
          console.warn(`[deployBackoffice] CNAME record type or requiredValue not found in domain status records.`);
        }
      } else {
        console.warn(`[deployBackoffice] Custom domain "${targetDomain}" not found in project custom domains response.`);
      }
    } catch (queryErr) {
      console.warn(`[deployBackoffice] Failed to query custom domain verification status or register CNAME:`, queryErr.message);
    }
  }
