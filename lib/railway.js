...
export async function deployBackoffice({ slug, planTipo, existingProjectId }) {
  ...
  // 2.2 — If not found by ID, search by name in the workspace (safety fallback)
  if (!projectRwId) {
    try {
      console.log(`[deployBackoffice] Checking if project "${projectName}" already exists in workspace...`);
      ...
      const matchedProject = existingProjects?.projects?.edges?.find(
        p => p.node.name === projectName
      )?.node;

      if (matchedProject) {
        projectRwId = matchedProject.id;
        const envEdge = matchedProject.environments?.edges?.find(e => e.node.name === "production") 
          ?? matchedProject.environments?.edges?.[0];
        environmentId = envEdge?.node?.id;
        console.log(`[deployBackoffice] Found existing project by name: "${projectName}" (ID: ${projectRwId}, env: ${environmentId}). Reusing...`);
      }
    } catch (err) {
      console.warn(`[deployBackoffice] Failed to search for projects by name:`, err.message);
    }
  }

  // 2.3 — If still not resolved, create a new project
  if (!projectRwId) {
  ...
  console.log(`[deployBackoffice] Step 5: Creating custom domain: "${slug}.clientesneurolinks.com"...`);
  try {
    await gql(`
  ...
  // 6 — Wait for deployment to finish and become SUCCESS
  console.log(`[deployBackoffice] Step 6: Waiting for deployment to finish successfully...`);
  let success = false;
...
export async function activateClientPortal(clienteId, adminDb) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let projectId = existingProjectId || crypto.randomUUID(); // Fallback ID
