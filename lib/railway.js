      ...
      if (existingProjectId && cliente.backoffice_activado) {
        ...
        try {
          await adminDb
            ...
            .upsert(
              { railway_project_id: existingProjectId, cliente_id: cliente.cliente_id },
          ...
          onProjectCreated: async (newProjectId) => {
            ...
            try {
              ...
              await adminDb
                .from('proyectos_railway')
                .upsert(
                  { railway_project_id: newProjectId, cliente_id: cliente.cliente_id, nombre_personalizado: cliente.proyecto_nombre || slug },
                  { onConflict: 'railway_project_id' }
                );
      ...
      if (projectId) {
        try {
          await adminDb
            .from('proyectos_railway')
            .upsert(
              { railway_project_id: projectId, cliente_id: cliente.cliente_id, nombre_personalizado: cliente.proyecto_nombre || slug },
              { onConflict: 'railway_project_id' }
            );
          console.log(`[activateClientPortal] Linked project ${projectId} to client ${cliente.cliente_id} in proyectos_railway`);
        } catch (linkErr) {
